...
- “只索引、只登记、不整理正文”完成导航与台账后停止。“加入知识库”在本流程中只指导航、台账、单文件结果和项目综合，不自动创建知识或记忆条目。
...
- **动作**：只识别稳定事实，以及未来有用且无法从代码直接推导的用户偏好、纠正反馈、项目决定、外部入口、已收敛方法、失败模式或护栏，报告“可另行保存为知识/记忆”的具体建议。本流程不直接创建、纠正或归档这些条目；同一请求明确要求保存时，也必须先完成适用文件阶段，且后续记录的来源仍回到原始文件，不能以 summary 代替。
...
- 本流程写入 `~/.agent-knowledge/README.md` 与 `indexes/` 下自己拥有的导航、台账、独立结果和综合；不写知识/记忆生命周期目录，不修改原文件。
